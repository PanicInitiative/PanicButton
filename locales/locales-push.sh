...
do
  ...
  do 
    ...
    cat $f | underscore --infmt text process 'data' | underscore process 'var lio = data.lastIndexOf("---"); var slc = data.slice(lio+1); if (slc.every(function(val){return val==""})) { } else { slc }' --outfmt text > ./build/$res/en/`basename $f`.txt
    ...
    if [[ -s ./build/$res/en/`basename $f`.txt ]] ; then
      ...
      cp ./build/$res/en/`basename $f`.json.tmp ./build/$res/en/`basename $f`.json
    fi;      
  ...
  do
    ...
    do 
      echo "Processing $f file.."; 
      cat $f | underscore --infmt text process 'data' | underscore process 'var lio = data.lastIndexOf("---"); var slc = data.slice(lio+1); if (slc.every(function(val){return val==""})) { } else { slc }' --outfmt text > ./build/$res/$lang/`basename $f`.txt
