...
do 
  ...
  cat $f | underscore --infmt text process 'data' | underscore process 'var lio = data.lastIndexOf("---"); var io =  data.indexOf("---"); if (lio != io) {data.slice(io+1,lio)} else {data.slice(0,lio)}' --outfmt text | yaml2json - | jq '.' > ./build/`basename $f`.json
